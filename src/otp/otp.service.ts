...
export class OtpService {
  ...
  private initializeEmailTransporter() {
    // Using Gmail SMTP for email OTP (same as your working Express app)
  ...
  private getOtpExpiry(): Date {
    ...
    expiry.setMinutes(expiry.getMinutes() + 5); // OTP expires in 5 minutes (same as your Express app)
    return expiry;
  }

  async sendEmailOtp(email: string): Promise<{ otpId: string; message: string }> {
    try {
      ...
      if (this.emailTransporter) {
        ...
        // Fallback: log OTP to console if email not configured
  ...
  async sendSmsOtp(phone: string): Promise<{ otpId: string; message: string }> {
    try {
  ...
  async verifyOtp(otpId: string, otp: string): Promise<{ isValid: boolean; message: string }> {
    try {
  ...
  async resendOtp(otpId: string): Promise<{ otpId: string; message: string }> {
    try {
      const otpRecord = await this.otpModel.findById(otpId);
